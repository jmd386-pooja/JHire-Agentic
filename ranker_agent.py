...
def main():
    """Main function to run the AI-powered resume processing and ranking."""
    try:
        # Check for API key
        ...
        print("🤖 Initializing AI-Powered Resume Processor...")
        processor = ResumeProcessor()
        
        # Example job description (you can modify this)
        job_description = """
        We are seeking a talented Data Scientist to join our growing team. The ideal candidate should have strong experience in machine learning, statistical analysis, and Python programming. Experience with TensorFlow, PyTorch, and scikit-learn is required. The role involves developing predictive models, analyzing large datasets, and communicating insights to stakeholders. A background in statistics and experience with SQL and data visualization tools is preferred.
        """
        ...
        if results.get('error'):
            ...
            print(f"\n✅ AI processing and ranking completed successfully!")
        
        if results['job_category']:
            print(f"🎯 Job categorized as: {results['job_category'].role_category}")
            print(f"🔍 Found {results['filtered_candidates']} relevant candidates")
            print(f"🏆 Top {len(results['final_scores'])} candidates ranked and scored")
            print(f"💾 Results stored in database: {results['stored_in_db']}")
            
            # Show top 3 candidates
            if results['final_scores']:
                print(f"\n🏅 TOP 3 CANDIDATES:")
                for i, candidate in enumerate(results['final_scores'][:3]):
                    print(f"{i+1}. {candidate.candidate_name} - Score: {candidate.final_score:.3f}, Rank: {candidate.final_rank}")
                    print(f"   Recommendation: {candidate.recommendation}")
                    print(f"   Strengths: {', '.join(candidate.strengths[:3])}")
                    print(f"   Areas for improvement: {', '.join(candidate.weaknesses[:2])}")
                    print()
        else:
            print("❌ Failed to categorize job description")
