...
import { Eye, Mail, Search, Calendar,LampDesk,Parentheses,Award} from 'lucide-react';
...
const CandidatesTable: React.FC = () => {
  ...
  const [collegeList, setCollegeList] = useState<string[]>([]); // College list for filter
  ...
  const fetchCandidates = async () => {
    ...
    try {
      ...
      // Fetching interview scores
      const interviewResponse = await fetch('/api/interviews');
      if (!interviewResponse.ok) throw new Error('Failed to fetch interview data.');
      const interviewData = await interviewResponse.json();
      
      const candidatesWithScores = candidatesData.data.map((candidate: Candidate) => {
        const interview = interviewData.find((interview: any) => interview.candidate_id === parseInt(candidate.id));
        const interviewScore = interview ? interview.Interviewscore : null;
        const Codingscore = interview ? interview.Codingscore : null;
        const interviewStatus =
  interviewScore !== null && Codingscore !== null ? (interviewScore + Codingscore) / 2 >= 80? "SELECTED": "NOT SELECTED": "N/A";
        
        return {
          ...candidate,
          Exam_date: candidate.Exam_date ? convertUTCToLocal(candidate.Exam_date) : null,
          Expiry: candidate.Expiry ? convertUTCToLocal(candidate.Expiry) : null,
          interviewDetails: interview || {},
          interviewStatus,  // Adding the interview status (PASS/FAIL)
        };
      });
      ...
      // Extract unique colleges from the candidates
  ...
  const sendInvitation = async (candidateId: string,candidatename:string, candidateEmail: string, candidateExam_URL: string, candidatetemp_name: string, candidatetempPassword: string, ExamDate: string) => {
    ...
    try {
      const response = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          id: candidateId,
          to: candidateEmail, // Replace with the recipient's email
  ...
  const sendBulkInvitations = async () => {
    ...
    try {
      ...
      // Clear selections after successful bulk invitation
  ...
  const filteredCandidates = candidates.filter((candidate) => {
    const matchesSearchQuery = [candidate.name, candidate.status, candidate.College]
      .some((field) =>
  ...
  // Sort the filteredCandidates before slicing
  ...
  const handleSave = async () => {
    ...
    try {
      setIsLoading(true);
      // Convert the Exam_date and Expiry to UTC before saving to the database
      if (editedCandidate.Exam_date) {
        editedCandidate.Exam_date = convertUTCToLocal(editedCandidate.Exam_date);
      }
      ...
      setIsEditing(false);
          ...
          // Show a success toast message for 2 seconds
          toast({
            title: "Success",
  ...
  useEffect(() => {
    fetchCandidates();
  }, [selectedCandidate,selectedCollege]);


const [selectedCandidate_exam, setSelectedCandidate_exam] = useState<Candidate | null>(null);
const handleInterviewDetailsClick = (candidate:any) => {
  ...
  return (
    <div className="container mx-auto px-4 py-4">
      ...
      <div className="grid gap-4">
        {currentCandidates
          .map((candidate) => (
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Interview Date: {new Date(candidate.Exam_date).toLocaleDateString()}
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                    ...
                    <HoverCardTrigger>
                     <span
                        className={`w-32 px-3 py-1 text-center inline-block rounded-full text-xs font-medium transition-all duration-500 ease-in-out transform 
                          ...
                          shadow-sm`}
                          // onClick={() => handleInterviewDetailsClick(candidate)}
                      >
                    ...
                    <HoverCardContent>
                        ...
                        <div className="space-y-3 mt-2">
                          <div className="flex items-center space-x-2">
                            ...
                            <p>
                              <strong className="text-gray-600">Interview Score:</strong>{" "}
                              <span className="font-medium text-blue-600">
                                {candidate.interviewDetails?.Interviewscore ?? "N/A"}
                              </span>
                            </p>
                          </div>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <p>
                              <strong className="text-gray-600">Coding Score:</strong>{" "}
                              <span className="font-medium text-green-600">
                                {candidate.interviewDetails?.Codingscore ?? "N/A"}
                              </span>
                            </p>
                          </div>
            ...
            {/* <button
              className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-500 ease-in-out transform 
                ${
                  candidate.interviewStatus === "PASS"
                    ? "bg-green-100 text-green-800 shadow-green-400 hover:shadow-lg hover:scale-105"
                    : candidate.interviewStatus === "FAIL"
                    ? "bg-red-100 text-red-800 shadow-red-400 hover:shadow-lg hover:scale-105"
                    : "bg-gray-100 text-gray-800 shadow-gray-400 hover:shadow-lg hover:scale-105"
                }
                shadow-sm`}
              onClick={() => {
                handleInterviewDetailsClick(candidate)}} // Handle click
            >
              {candidate.interviewStatus}
            </button> */}
                  ...
                  <Button
                    ...
                    disabled={loadingId === candidate.id || candidate.status !== "PENDING"}
                  >
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <span>{`Page ${currentPage} of ${Math.ceil(filteredCandidates.length / itemsPerPage)}`}</span>
        <div className="flex items-center space-x-2">
            ...
            <div className="grid gap-4 py-4">
              {[
                ...
                <div key={key} className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input
                    id={key}
                    type={['Exam_date', 'Expiry'].includes(key) ? 'datetime-local' : 'text'}
                    value={isEditing ? (editedCandidate as any)[key] : (selectedCandidate as any)[key]}
                    onChange={(e) =>
                      isEditing &&
 ...
 {/* {selectedCandidate_exam && (
              <Dialog open={!!selectedCandidate_exam} onOpenChange={handleModalClose}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Interview Details for {selectedCandidate_exam.name}</DialogTitle>
                    <DialogDescription>
                      Review the candidate's interview details below.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="space-y-4">
                    <p>
                      <strong>Interview Score:</strong> {selectedCandidate_exam.interviewDetails?.Interviewscore ?? 'N/A'}
                    </p>
                    <p>
                      <strong>Coding Score:</strong> {selectedCandidate_exam.interviewDetails?.Codingscore ?? 'N/A'}
                    </p>
                    <p>
                      <strong>Interview Status:</strong> {selectedCandidate_exam.interviewStatus}
                    </p>
                  </div>
                  <DialogFooter>
                    <button
