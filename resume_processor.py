...
class ResumeProcessor:
    def __init__(self, db_path: str = "./chroma_db"):
        """Initialize the resume processor with AI-powered extraction and vector database."""
    ...
    def process_resume(self, resume_text: str, candidate_name: str, candidate_id: str = None) -> str:
        ...
        try:
            ...
            print(f"   Skills Found: {len(extracted_data.programming_languages + extracted_data.frameworks + extracted_data.databases + extracted_data.cloud_platforms + extracted_data.devops_tools + extracted_data.other_technologies)}")
    ...
    def process_multiple_resumes(self, resumes: List[Dict[str, Any]]) -> List[str]:
    ...
    def get_total_resumes(self) -> int:
        ...
        try:
            ...
            # Try to get collection count first
            try:
            ...
            # If we have documents, get them using get() instead of similarity_search to avoid empty embedding
            try:
                # Use the collection's get() method directly
                results = self.vectorstore._collection.get()
                
                if not results or not results.get('metadatas'):
                    return 0
                
                # Count unique candidates from metadata
    ...
    def get_all_candidates(self) -> List[Dict[str, Any]]:
        """Get all candidates with their resume information."""
        try:
            # First check if vectorstore exists
            if not hasattr(self, 'vectorstore') or not self.vectorstore:
                return []
            ...
            # If we have documents, get them using get() instead of similarity_search
            try:
                # Use the collection's get() method directly
                results = self.vectorstore._collection.get()
    ...
    def clear_database(self):
        ...
        try:
            # Method 1: Delete all documents by ID
            try:
                results = self.vectorstore._collection.get()
    ...
    def reset_database(self):
        ...
        try:
            ...
            # Remove directory with individual file deletion
            if os.path.exists(self.db_path):
                max_retries = 10
                for attempt in range(max_retries):
                    try:
